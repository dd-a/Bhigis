...
function getColorFromEcart(ecart, minEcart) {
    const maxEcart = 20; // Écart considéré comme "mauvais"
    const ratio = Math.min((ecart - minEcart) / maxEcart, 1); // de 0 (bon) à 1 (mauvais)

    // Interpolation entre vert (0) et rouge (1)
    const r = Math.round(255 * ratio);
    const g = Math.round(180 * (1 - ratio)); // vert moins saturé
    const b = 0;
...
async function search() {
    ...
    data.forEach((item, index) => {
		const ecart = Math.abs(item.époque - yearInput);
		    ...
		    // Générer une couleur dynamique selon l’écart
