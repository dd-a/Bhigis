...
async function search() {
    ...
    [...data].reverse().forEach((item, index) => {
        ...
        // Calcule le vrai index (ordre original de la liste)
        const originalIndex = data.length - 1 - index;

        const icon = L.divIcon({
            className: 'custom-div-icon',
            html: `<div style="background:${color};color:${textColor};border-radius:50%;width:24px;height:24px;text-align:center;line-height:24px;">${originalIndex + 1}</div>`,
            iconSize: [24, 24],
            iconAnchor: [12, 12]
        });
        ...
        `);
        markers[originalIndex] = marker; // Réutilisé dans la liste
    });
