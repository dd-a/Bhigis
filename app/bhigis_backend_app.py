...
# ============================================================
# Import conditionnel de la config DB
# ============================================================
...
if use_tunnel:
    try:
        ...
        raise ImportError(
            "Le module 'db.db_config' est requis pour l'utilisation avec tunnel SSH "
            "(défini par USE_SSH_TUNNEL=1)."
        )
else:
    # Sur le serveur : pas de tunnel, connexion directe
    from db_config import get_db_connection

# ============================================================
# Fonction d'ouverture de tunnel ssh si nécessaire (selon le serveur)
# ============================================================
def get_connection():
    """
...
translations = {
    "fr": {
        ...
        "step2": "2. Déclencher et recevoir les résultats :",
    ...
    "nl": {
        ...
        "step2": "2. Start en ontvang de resultaten:",
...
# ============================================================
def increment_geocode_counter(mode="simple"):
    """
    Incrémente le compteur d'appels dans /opt/bhigis/geocode_counter.txt
    mode = "simple" ou "batch"
    """
    count_file = "../geocode_counter.txt"

    # Valeurs par défaut
    counts = {"simple": 0, "batch": 0}
    ...
    # Incrémenter le compteur du mode demandé
...
def geocode():
    ...
    if address:
        if address:
            increment_geocode_counter("simple")
        conn, tunnel = get_connection()
        conn.autocommit = True      #### A commenter !!! ###
        cur = conn.cursor()
        try:
...
#        import logging
        ...
        cur.execute(
            ...
            ORDER BY best_answer DESC, sim DESC, commune_match, dist_muni
            LIMIT 12
            """,
            (commune,),
        )
...
#            logging.warning(f"Colonnes reçues : {row}")
...
#        finally:
        ...
        if tunnel:
            tunnel.stop()
#    logging.warning("JSON envoyé = %s", results)
...
def generate_results_zip(base_filename, gdf1, df2, df3):
    ...
    # Couche principale
    gdf1.to_file(gpkg_filename, driver="GPKG", layer="results")
    
    ########
    #import sqlite3

    print("📂 Vérification du GeoPackage après écriture de gdf1...")
    ...
    try:
        ...
        # Vérifier structure de la table 'resultats'
        if "resultats" in tables or "results" in tables:
            table_name = "results" if "results" in tables else "resultats"
            print(f"✔ Vérification des colonnes dans {table_name}...")
            cur.execute(f"PRAGMA table_info('{table_name}')")
            cols = cur.fetchall()
            print(f"📌 Colonnes trouvées : {cols if cols else '❌ Aucune colonne !'}")
        ...
        print(f"❌ Erreur lors de la vérification du GPKG : {e}")
    ####################

    # Tables secondaires
    conn = sqlite3.connect(gpkg_filename)
    df2.to_sql("not_found", conn, if_exists="replace", index=False)
    df3.to_sql("suggestions", conn, if_exists="replace", index=False)
    conn.close()
    
    # 📌 Appliquer automatiquement les styles QGIS au Geopackage
    apply_qgis_styles_to_gpkg(gpkg_filename)
...
# ============================================================
# Génération de geopackage avec styles intégrés
# ============================================================

def apply_qgis_styles_to_gpkg(gpkg_path):
    """
    🔹 Renomme les couches et insère les styles QML dans la table layer_styles du GeoPackage.
    🔹 Couch es renommées :
        - 'resultats' → 'results'
        - 'données_non_trouvées' → 'not_found'
    🔹 Styles appliqués automatiquement si les fichiers .qml existent dans le dossier.
    """
    try:
        ...
        # 1️⃣ Assurer l'existence de la table layer_styles
        cur.execute("""
        ...
        # 2️⃣ Renommer les couches dans la table metadata du GPKG
        # rename_map = {
 #            "resultats": "results",
 #            "données_non_trouvées": "not_found"
 #        }
        ...
        # 3️⃣ Appliquer les styles QML
        styles = [
            ("results", "results_style.qml"),
            ("not_found", "not_found_style.qml")
        ]

        for layer_name, qml_file in styles:
            if os.path.exists(qml_file):
                with open(qml_file, "r", encoding="utf-8") as f:
                    qml_data = f.read()

                # Supprimer un ancien style pour éviter les doublons
                cur.execute(
                ...
                cur.execute("""
                    ...
                    VALUES (?, 'geom', ?, ?, 1)
                """, (layer_name, f"{layer_name}", qml_data))
#                """, (layer_name, f"{layer_name}_style", qml_data))

        conn.commit()
        conn.close()
        print(f"✅ Styles intégrés dans {gpkg_path}")
    except Exception as e:
        print(f"⚠️ Erreur intégration styles QGIS : {e}")
...
def analyze_file():
    ...
    try:
        ...
        app.logger.exception("Erreur batch : %s", e)
...
def batch_geocode():
    ...
    increment_geocode_counter("batch")
    ...
    try:
        ...
        try:
            ...
            app.logger.info(f"Lecture résultats sur {db_user}@{db_host}:{db_port}/{db_name}")

            engine_url = f"postgresql://{db_user}:{db_password}@{db_host}:{db_port}/{db_name}"
            engine = create_engine(engine_url)

            # Lecture avec logs intermédiaires
            app.logger.info("Lecture de adress_geoloc_pt...")
            gdf1 = gpd.read_postgis(f"SELECT * FROM {schema_name}.adress_geoloc_pt", engine, geom_col="geom")
            app.logger.info(f"  → {len(gdf1)} lignes")

            app.logger.info("Lecture de missing...")
            df2 = pd.read_sql(f"SELECT * FROM {schema_name}.missing", engine)
            app.logger.info(f"  → {len(df2)} lignes")

            app.logger.info("Lecture de online_suggest...")
            df3 = pd.read_sql(f"SELECT * FROM {schema_name}.online_suggest", engine)
            app.logger.info(f"  → {len(df3)} lignes")

        except Exception as e:
            app.logger.error(f"Erreur lors de la lecture des résultats : {e}", exc_info=True)
            return f"Erreur lors de la lecture des résultats : {str(e)}", 500
        ...
        app.logger.info(f"Taille du fichier ZIP : {os.path.getsize(zip_path)/1024:.1f} Ko")
        app.logger.info(f"Envoi du fichier ZIP : {zip_path}")
        
        import logging
        logging.info(f"Envoi du fichier ZIP : {os.path.basename(zip_path)}")
        return send_file(
            zip_path,
            as_attachment=True,
        ...
        app.logger.info("✅ Fichier envoyé avec succès")
...
# ============================================================
# Accès au compteur par http://127.0.0.1:8080/stats
# ============================================================
